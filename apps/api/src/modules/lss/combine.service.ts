...
//                                 head branch, moves the first 20 held slots
...
import { LSS_FILL_WINDOW_DAYS, LSS_SLOTS_PER_ROOM } from './rooms.service';
...
export const CombineService = {
  ...
  async assertHeadBranchAdmin(db: Pool, userId: string): Promise<{ headBranchId: string }> {
    ...
    if (res.rows.length === 0) throw new ForbiddenError('User not found', 'LSS_HEAD_PERM');
    const { branch_id, role, is_head } = res.rows[0];
    if (role !== 'branch_admin' || !is_head || !branch_id) {
      throw new ForbiddenError('Only branch admins at the head branch can perform this action', 'LSS_HEAD_PERM');
    }
  ...
  async combineRooms(
    ...
    if (sourceRoomIds.length < 2) {
      throw new ValidationError('Need at least 2 source rooms to combine', 'LSS_COMBINE_TOO_FEW');
    }
    return runInTransaction(db, async (client) => {
      const sortedIds = [...sourceRoomIds].sort();
      const roomsRes = await client.query(
        `SELECT id, plan_id, status, branch_id
         FROM lss_rooms
         WHERE id = ANY($1::uuid[])
         ORDER BY id
         FOR UPDATE`,
      ...
      for (const r of roomsRes.rows) {
        if (r.status !== 'pending_combine') {
          throw new ValidationError(
            ...
            'LSS_COMBINE_BAD_STATUS'
          );
        }
      }

      const slotsRes = await client.query<{ id: string; room_id: string }>(
        `SELECT id, room_id FROM lss_slots
         WHERE room_id = ANY($1::uuid[]) AND status = 'held'
         ORDER BY paid_at ASC, id ASC
         FOR UPDATE`,
        [sortedIds]
      );
      if (slotsRes.rows.length < LSS_SLOTS_PER_ROOM) {
        throw new ValidationError(
          `Source rooms have only ${slotsRes.rows.length} held slot(s); need ${LSS_SLOTS_PER_ROOM}`,
          'LSS_COMBINE_NOT_ENOUGH_SLOTS'
        );
      }
      const slotsToMove = slotsRes.rows.slice(0, LSS_SLOTS_PER_ROOM);
      ...
      const newRoomRes = await client.query(
        `INSERT INTO lss_rooms
           (plan_id, branch_id, room_number, is_combined, status, fill_deadline, created_by, notes)
         VALUES ($1, $2, $3, true, 'filling', now() + INTERVAL '${LSS_FILL_WINDOW_DAYS} days', $4, $5)
         RETURNING *`,
        [planId, headBranchId, roomNumber, actorUserId, notes ?? null]
      );
      const newRoom = newRoomRes.rows[0];

      for (let i = 0; i < slotsToMove.length; i++) {
        await client.query(
          `UPDATE lss_slots SET room_id = $1, slot_number = $2 WHERE id = $3`,
  ...
  async refundRoom(db: Pool, roomId: string, headBranchId: string, reason?: string): Promise<{ slotsRefunded: number }> {
    return runInTransaction(db, async (client) => {
      const roomRes = await client.query<{ status: string; branch_id: string }>(
        `SELECT status, branch_id FROM lss_rooms WHERE id = $1 FOR UPDATE`,
        [roomId]
      );
      ...
      if (roomRes.rows[0].branch_id !== headBranchId) {
        throw new ForbiddenError('Room is not owned by the head branch', 'LSS_ROOM_WRONG_BRANCH');
      }
      assertTransition(roomRes.rows[0].status as any, 'expired');

      const refundRes = await client.query(
        `UPDATE lss_slots SET status = 'refunded'
