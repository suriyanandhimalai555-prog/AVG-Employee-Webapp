...
export const BuildersService = {
  ...
  async createPlan(
    ...
    // cash_gpay split — same server-side sum check
    if (payload.lumpSumMode === 'cash_gpay' &&
        Math.abs((payload.lumpSumCashAmount ?? 0) + (payload.lumpSumGpayAmount ?? 0) - pkg.investmentAmount) > 0.01) {
      throw new ValidationError('lumpSumCashAmount + lumpSumGpayAmount must equal the package investment amount');
    }
      ...
      const insertResult = await client.query(
        `INSERT INTO builders_plans
           (branch_id, customer_id, package_number,
            ...
            lump_sum_cash_amount, lump_sum_bank_amount, lump_sum_gpay_amount)
        ...
        [
          ...
          (payload.lumpSumMode === 'cash_bank' || payload.lumpSumMode === 'cash_gpay') ? payload.lumpSumCashAmount : null,
          payload.lumpSumMode === 'cash_bank' ? payload.lumpSumBankAmount : null,
          // cash_gpay split: gpay portion — null unless mode is cash_gpay
          payload.lumpSumMode === 'cash_gpay' ? payload.lumpSumGpayAmount : null,
        ]
      );
      ...
      try {
        const insertResult = await client.query(
          `INSERT INTO builders_payouts
             (plan_id, month_number, amount, payout_date, payment_mode, proof_key, transaction_id, cash_amount, bank_amount, gpay_amount, notes, entered_by)
  ...
  async correctPlan(
    ...
    return runInTransaction(db, async (client: PoolClient) => {
      ...
      if (payload.lumpSumMode === 'cash_bank') {
        ...
        fields.push(`lump_sum_gpay_amount = NULL`);
      } else if (payload.lumpSumMode === 'cash_gpay') {
        if (payload.lumpSumCashAmount != null) { fields.push(`lump_sum_cash_amount = $${idx++}`); vals.push(payload.lumpSumCashAmount); }
        if (payload.lumpSumGpayAmount != null) { fields.push(`lump_sum_gpay_amount = $${idx++}`); vals.push(payload.lumpSumGpayAmount); }
