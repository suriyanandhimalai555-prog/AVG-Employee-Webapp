      ...
      <div className="px-6 md:max-w-3xl">
        ...
        <div className="mb-1.5">
          <div className="relative">
            ...
            <input
              ...
              className={`${INPUT_BASE} pl-11 pr-10`}
            />
            <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
              {isFetching && <Loader2 size={14} className="text-indigo animate-spin" />}
              {searchInput && (
                <button
                  type="button"
                  onClick={() => setSearchInput('')}
                  aria-label="Clear search"
                  className="w-5 h-5 rounded-full flex items-center justify-center text-navy/40 hover:text-navy/70 hover:bg-navy/8 transition-all"
                >
                  <X size={12} />
                </button>
              )}
        ...
        <div className="mb-3 min-h-[1.25rem]">
          {!isLoading && !isError && total > 0 && (
            <p className="text-[11px] font-medium text-navy/60 ml-1">
              {total <= PAGE_SIZE
                ? `${total} customer${total !== 1 ? 's' : ''}`
                : `${rangeStart}–${rangeEnd} of ${total} customers`}
        ...
        {isManagement && !branchId ? (
          ...
          /* Single container with hairline dividers — no per-row card-shadow stacking */
          <div className="bg-white rounded-2xl card-shadow border border-border divide-y divide-border overflow-hidden">
            {customers.map((c) => {
              ...
              return (
                <div
                  key={c.id}
                  className={`flex items-center gap-3 px-4 py-3.5 transition-colors duration-700 ${
                    flashing ? 'bg-emerald/8' : 'bg-white'
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {hasPhone ? (
                        <>
                          <Phone size={10} className="text-navy/40 shrink-0" />
                          <p className="text-[11px] font-medium text-navy/60 truncate">{c.phone}</p>
                        </>
                      ) : (
                        <button
                          type="button"
                  ...
                  {/* Pencil — 44px touch target */}
                  <button
                    type="button"
                    onClick={() => openEdit(c)}
                    aria-label={`Edit contact details for ${c.name}`}
                    className="w-11 h-11 rounded-xl flex items-center justify-center text-navy/30 hover:text-navy/60 hover:bg-navy/5 transition-all duration-200 tactile-press shrink-0"
                  >
                    <Pencil size={15} />
                  </button>
                  ...
                  <button
                    ...
                    className="w-11 h-11 flex items-center justify-center shrink-0 tactile-press disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    {saving
                      ? <Loader2 size={28} className="animate-spin text-navy/30" />
                      : c.has_whatsapp
                        ? <ToggleRight size={30} className="text-emerald" />
      ...
      >
        <form onSubmit={handleEditSave} className="space-y-5">
          <div className="space-y-1.5">
            ...
            <p className="text-[10px] text-navy/60 ml-1">
