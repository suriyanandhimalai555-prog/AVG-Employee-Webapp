    ...
    try {
      if (payMode === 'deposit') {
        const res = await createPending({
          schemeCode: 'lss_scheme',
          customerId: customer.id,
          referrerId: form.referrerId || undefined,
          branchId:   isManagement ? branchId : undefined,
          payload: { planId: form.planId, quantity: effectiveQuantity, notes: form.notes.trim() || undefined },
          firstPayment: {
  ...
  return (
    <SchemePageWrapper>
      ...
      <form onSubmit={handleSubmit} className="px-4 space-y-5">
        ...
        <button
          type="submit"
          disabled={isLoading || creatingPending || !selectedPlan}
