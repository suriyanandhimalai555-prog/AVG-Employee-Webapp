// LSS Scheme landing page.
// Sections:
//   1. Branch summary strip (total slots / held / won / collected / commission)
//   2. Room status filter chips
//   3. Room list — each card shows plan, room number, status, fill progress
//   4. Plans reference (the 5 price points seeded in migration 038)

import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useSelector } from 'react-redux';
import { Loader2, Layers, Users, CheckCircle2, AlertTriangle, Clock, Plus, ShieldCheck } from 'lucide-react';
  ...
  return (
    <SchemePageWrapper>
      <SchemePageHeader
        ...
        title="LSS Scheme"
        subtitle="One-time investment · 20-slot rooms · monthly draws on the 12th"
        action={(canSell || canSeeHeadBranch) ? (
          <div className="flex items-center gap-2">
            {canSeeHeadBranch && (
              <button
                type="button"
                onClick={() => navigate('/money/schemes/lss/head')}
            ...
            {canSell && (
              <button
                type="button"
                onClick={() => navigate('/money/schemes/lss/add')}
                aria-label="Sell an LSS slot"
                className="w-10 h-10 rounded-2xl bg-gradient-to-br from-violet-500 to-fuchsia-600 flex items-center justify-center text-white shadow-md tactile-press"
              >
          ...
          <div className="space-y-3">
            {rooms.map((room) => {
              ...
              const progressPct = room.status === 'active' || room.status === 'completed'
                ? (draws / SLOTS_PER_ROOM) * 100
                : (filled / SLOTS_PER_ROOM) * 100;
                  ...
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div>
                      <p className="text-sm font-bold text-navy">
                        {room.plan_name} · Room #{room.room_number}
                        {room.is_combined && (
                      ...
                      <p className="text-xs text-navy/40 mt-0.5">
                        {room.branch_name} · {formatCurrency(parseFloat(room.plan_price))}
                      </p>
                    </div>
                  ...
                  <div className="flex items-center justify-between text-[11px] text-navy/50 font-medium mb-1.5">
                    <span>
                      {room.status === 'active' || room.status === 'completed'
                        ? `${draws}/${SLOTS_PER_ROOM} draws done`
                        : `${filled}/${SLOTS_PER_ROOM} slots filled`}
                    </span>
      ...
      {/* Plans reference — level payout preview */}
      <div className="px-4 mb-8">
        <p className="text-[10px] font-bold text-navy/40 uppercase tracking-widest mb-3">Plans &amp; Payouts</p>
        {plansLoading ? (
          ...
          <div className="bg-white rounded-2xl card-shadow border border-navy/5 overflow-hidden">
            ...
            <div className="divide-y divide-navy/5">
              {plans.map((p) => {
                ...
                const total  = p.price * 35.25;
                return (
                  <div key={p.id} className="grid grid-cols-[1fr_1fr_1fr_1fr] gap-2 px-4 py-2.5 items-center">
                    <span className="text-sm font-bold text-violet-700">{formatCurrency(p.price)}</span>
                    <span className="text-xs font-semibold text-navy text-right">{formatCurrency(draw1)}</span>
                    <span className="text-xs font-semibold text-navy text-right">{formatCurrency(draw20)}</span>
                    <span className="text-xs font-semibold text-fuchsia-700 text-right">{formatCurrency(total)}</span>
                  </div>
                );
              })}
            </div>
