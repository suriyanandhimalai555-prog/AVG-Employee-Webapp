// Head-branch admin console for the Gold Coin scheme.
//
// Backend already enforces "branch_admin at a branch with is_head_branch = true"
// via CombineService.assertHeadBranchAdmin. This page is the UI surface for the
// two head-branch-only actions that were otherwise unreachable:
//
//   - Combine: pick N pending_combine rooms inside the SAME package, fold
//     the first 16 held slots into a new room owned by the head branch.
//   - Refund: refund every held slot of one pending_combine room (room → expired).
//
// Read-only fallback: MD / Director see the same dashboard without action
// buttons, so leadership can audit the queue.
//
// Decoupling notes:
//   - Uses ONLY the apiSlice mutations already wired up to the backend routes.
//   - No new endpoints, no scheme-specific store slice, no shared mutations.
//   - Each action confines its side effects to a single mutation call.
...
// MD and Director can read; only head-branch admin can act.
...
// ─── Per-room row with refund / extend actions ──────────────────────────────

const PendingRoomRow = ({
  room,
  selectable,
  selected,
  onToggle,
...
// ─── Card per package: a group of pending_combine rooms with the same package_id ─
      ...
      <div className="px-4 py-3 border-b border-navy/5 flex items-center justify-between bg-navy/[0.02]">
        <div>
          ...
          <p className="text-[11px] text-navy/40">
            {formatCurrency(parseFloat(rooms[0].package_price))} / slot · {rooms[0].package_gold_grams}GM ·
        ...
        {selectable && (
          <button
            ...
            className="px-3 py-1.5 rounded-full bg-gradient-to-br from-yellow-500 to-amber-600 text-white text-[11px] font-bold tactile-press disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1.5"
          >
...
export const GoldCoinHeadBranchPage = () => {
  ...
  const {
    ...
    refetch,
  } = useGetGoldCoinAwaitingCombineQuery(undefined, { skip: !canView });
  const { data: packages = [] } = useGetGoldCoinPackagesQuery(undefined, { skip: !canView });

  const [combineRooms,  { isLoading: combining }] = useCombineGoldCoinRoomsMutation();
  const [refundRoom,    { isLoading: refunding }] = useRefundGoldCoinRoomMutation();
  ...
  // Memoise to stabilise the dependency identity for downstream useMemo hooks —
  // otherwise the [] literal recreates on every render and forces re-grouping.
  const rooms = useMemo(() => awaitingCombineRooms ?? [], [awaitingCombineRooms]);

  const groupedByPackage = useMemo(() => {
    const map = new Map();
    for (const room of rooms) {
      const list = map.get(room.package_id) ?? [];
      list.push(room);
      map.set(room.package_id, list);
    }
    return Array.from(map.entries()).map(([packageId, packageRooms]) => ({
      packageId,
  ...
  const onCombine = async (roomsInGroup) => {
    ...
    if (sourceRoomIds.length < 2) {
      setError('Pick at least 2 rooms from the same package to combine.');
      return;
    }
    const reason = window.prompt(
      `Combine ${sourceRoomIds.length} rooms?\n\nThe first 16 held slots (earliest paid first) will be moved into a new room owned by your branch. Source rooms with zero remaining slots will be marked combined.\n\nOptional notes for the combined room:`,
      '',
    );
      ...
      <div className="px-4 mb-10">
        {roomsLoading ? (
          ...
          <div className="bg-white rounded-2xl p-8 card-shadow border border-navy/5 text-center">
            ...
            <button
              type="button"
              onClick={() => navigate('/money/schemes/gold-coin')}
...
export default GoldCoinHeadBranchPage;
